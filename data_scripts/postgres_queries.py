...
def run_queries(landname, pgdatabase, pguser, pghost, pgpassword):
    ...
    for chunk in ids:
        ...
        if result_check == 0:
            print("Chunk number {0} of {1} is empty, setting water = 100 procent".format(chunk, len(ids)))
            ...
            print("Chunk number {0} of {1} is not empty, Processing...".format(chunk, len(ids)))
    ...
    for chunk in ids:
        ...
        if result_check == 0:
            print("Chunk number {0} of {1} is empty, moving to next chunk".format(chunk, len(ids)))
            conn.rollback()
        else:
            conn.commit()
            print("Chunk number {0} of {1} is not empty, Processing...".format(chunk, len(ids)))
            ...
            SELECT Distinct ON (chunk_nr{1}.id) chunk_nr{1}.id as id,
